...
async def add_message(data: MessageRequestAdd):
    async with async_session_maker() as session:
        latest_message = await MessagesRepository(session).get_one_or_none(
        ...
        if latest_message:
            ...
            _data = MessageAdd(
                **data.model_dump(),
                ...
                user_message_count=new_message_count,
            )
        else:
            _data = MessageAdd(
                **data.model_dump(), created_at=created_at, user_message_count=1
            )
